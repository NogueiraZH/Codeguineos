...
def on_canvas_configure(event):
    canvas_scroll.itemconfig("frame_window", width=event.width)

def criar_linha(frame_pai, chave, valor, largura=20):
    row = tk.Frame(frame_pai, bg="#ADD8E6")
    row.pack(fill="x", expand=True, pady=5)

    tk.Label(row, text=chave, bg="#ADD8E6", anchor="center",
             font=("Arial", 10, "bold")).pack(side="top", fill="x", padx=5)

    label_result = tk.Label(row, text="--", bg="white", anchor="center",
                            relief="sunken", font=("Arial", 12))
    label_result.pack(side="top", fill="x", padx=5)
...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("CANector")
    centralize(root, 1000, 600)

    # Frame principal com grid
    # ... [mantém todo o código anterior até a criação do main_frame]
    ...
    # Coluna 2 - Dados adicionais com scroll
    frame_scroll = tk.Frame(main_frame, bg="#ADD8E6", padx=0, pady=0)
    frame_scroll.grid(row=0, column=1, sticky="nsew")

    canvas_scroll = tk.Canvas(frame_scroll, bg="#ADD8E6", highlightthickness=0)
    ...
    frame_adicionais = tk.Frame(canvas_scroll, bg="#ADD8E6", padx=10, pady=10)
    canvas_window = canvas_scroll.create_window((0, 0), window=frame_adicionais, anchor="nw", tags="frame_window")



    frame_adicionais.bind("<Configure>", on_frame_configure)
    ...
    # Função modificada para preencher largura e alinhar labels


    #--------------------------------------------------------------------
    # Criação dos frames e preenchimento dos dados
    # -------------------------------------------------------------------
